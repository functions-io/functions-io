...
function process(req, res, next, mountpath){
    try{
        ...
        req.factory.invokeMessage(messageRequest, function(messageResponse){
            ...
            if ((messageResponse.returnType) && (messageResponse.returnType === "HTTP")){
                ...
                if (messageResponse.error){
                    ...
                    if (messageResponse.result.code){
                        res.writeHead(messageResponse.result.code, messageResponse.result.headers);
                    }
                    else{
                        res.writeHead(200, messageResponse.result.headers);    
                    }
                ...
                responseHeaders["Access-Control-Allow-Headers"] = "Content-Type, api_key, Authorization, x-requested-with, Total-Count, Total-Pages, Error-Message";
                responseHeaders["Access-Control-Allow-Methods"] = "POST, GET, DELETE, PUT, OPTIONS";
