...
function process(req, res, next, factory, config){
    try{
        var messageRequest = {};
        var tempArrayURL;
        ...
        if ((config.cors) && (config.cors.enable)){
            responseHeaders = appendCORSHeaders(responseHeaders, config.cors.fromOrigin);
        }
        ...
        factory.invokeMessage(messageRequest, function(messageResponse, functionManager){
            if (functionManager){
...
module.exports = function(req, res, next, factory, config){
    try{
        if (req.body){
        ...
        else
        {
            ...
            if ((req.method === "POST") || (req.method === "PUT")){
                req.on("data", function(chunk) {
                    ...
                    process(req, res, next, factory, config);
                });
            }
            else{
                process(req, res, next, factory, config);
