var staticweb = require("./staticweb");
...
module.exports = function(factory, req, res){
    try{
        var body = [];
        req.body = null;

        if (factory.isPublicCatalog){
            if (req.headers["content-type"] !== "application/json"){
                if (req.url.indexOf("/admin") === 0){
                    staticweb("/admin", "htmlFiles", req, res);
                    return;
                }
                else if (req.url === "/"){
        ...
        if (req.method === "POST"){
            req.on("data", function(chunk) {
                ...
                processRequest(factory, req, res);
            });
        }
...
function processRequest(factory, req, res){
    try{
        var messageRequest = {};
        var tempArrayURL;
        ...
        factory.invokeMessage(messageRequest, function(messageResponse){
