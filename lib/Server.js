...
var Server = function(config){
    ...
    if ((this.config.db.provider === undefined) || (this.config.db.provider === null)){
        this.config.db.provider = "sys.db.provider.mongo";
    }
    if ((this.config.db.url === undefined) || (this.config.db.url === null)){
        this.config.db.url = "mongodb://localhost:27017/sample";
    }

    this.server = functions_io_core.createServer(this.config);
    this.server.factory.global.config = {};
    this.server.factory.global.config.db = this.config.db;
