var ServerProcessHTTP = function(factory){
    ...
    this.process = function(req, res){
        try{
            ...
            if (req.method === "POST"){
                ...
                res.end(JSON.stringify(this.getJSONRPCErrorObject(-32600, "Method Not Allowed")));
            }
        }
        catch(errHTTP){
            res.writeHead(500, {"Content-Type": "application/json"});
            res.end(JSON.stringify(this.getJSONRPCErrorObject(-32000, errHTTP.message)));
        }
    };

    this._process = function(req, res){
        try{
            var messageRequest = {};
            var tempArrayURL;
            ...
            if (req.body){
                try{
                    ...
                    res.end(JSON.stringify(this.getJSONRPCErrorObject(-32700, "Parse error")));
                    return;
                }
            }
            ...
            /***Retirar***/
            console.log("Invoke method " + messageRequest.method + " - " + messageRequest.version);

            this.factory.invokeMessage(messageRequest, function(messageResponse){
            ...
            if (errRequest instanceof ReferenceError){
                httpCode = 404;
                responseJSON = this.getJSONRPCErrorObject(-32601, errRequest.message);
            }
            else if (errRequest instanceof RangeError){
                httpCode = 400;
                responseJSON = this.getJSONRPCErrorObject(-32602, errRequest.message);
            }
            else{
                httpCode = 500;
                responseJSON = this.getJSONRPCErrorObject(-32603, errRequest.message);
            }
    ...
    this.getJSONRPCErrorObject = function(code, message, id){
        var errorJSONRPC = {};
        if (id){
            errorJSONRPC.id = id;
        }
...
module.exports = ServerProcessHTTP;